  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Filter to only include cabs with status exactly "assigned" (case sensitive)
        const currentlyAssignedCabs =
          assignedCabsRes.status === "fulfilled"
            ? assignedCabsRes.value.data.filter((cab) => cab.status === "assigned")
            : []
